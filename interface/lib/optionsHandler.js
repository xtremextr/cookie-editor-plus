...
export class OptionsHandler extends EventEmitter {
  ...
  getAnimationsEnabled() {
    // If options haven't been loaded yet, return default true
  ...
  onMessage = async (request) => {
    ...
    switch (request.type) {
      case 'optionsChanged': {
        // Check for params (new structure) or data (old structure)
        const fromGuid = (request.params && request.params.from) || (request.data && request.data.from);
        if (fromGuid === this.guid) {
          return;
        }
